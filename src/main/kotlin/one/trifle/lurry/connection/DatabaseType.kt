 ...
 */
enum class DatabaseType constructor(private val driver: String, private val type: String, val mixed: Class<*>) {
    ...
    companion object {
        @JvmStatic fun of(name: String): DatabaseType = if (name.contains('.')) {
            values().find { type -> type.driver.equals(name, ignoreCase = true) }
        } else {
            values().find { type -> type.type.equals(name, ignoreCase = true) }
        } ?: DEFAULT
    }
}
